# BACKEND_ARCHITECTURE.md — ActionLens AI Architecture Specification
> **Status**: Technical Reference & Schema Contract  
...
│  - Authentication (`@supabase/ssr` cookies)                            │
...
└────────────────────────────────────┬────────────────────────────────────┘
           ...
           │                                                   │
┌──────────▼──────────────────────────┐             ┌──────────▼──────────────────────────┐
│      SUPABASE PAAS INFRASTRUCTURE   │             │       FASTAPI AI MICROSERVICE       │
│  - PostgreSQL + pgvector Extension  │             │  - OpenAI GPT-4o Integration        │
│  - Storage Buckets (reports/avatars)│             │  - Structured Outputs (JSON Schema) │
│  - Realtime WebSocket Channels      │             │  - RAG Vector Indexing Pipeline     │
│  - Row-Level Security (RLS) Policies│             │  - Server-Sent Events (SSE Streaming)│
└─────────────────────────────────────┘             └─────────────────────────────────────┘
```
...
## 3. API Directory Contract

### Authentication APIs
* `POST /api/auth/register` — Create user credential & profile record.
* `POST /api/auth/login` — Create SSR session cookie.
* `POST /api/auth/logout` — Destroy session cookie.
* `GET /api/auth/me` — Fetch current user context.
* `PATCH /api/user/onboarding` — Save onboarding preferences.

### Dashboard & Analytics APIs
* `GET /api/dashboard/summary` — Fetch current active alert, river metrics, & quick stats.
* `GET /api/analytics/metrics` — Aggregate response efficiency & trend metrics.

### AI Recommendations APIs
* `GET /api/recommendations` — Fetch personalized operational checklist.
* `POST /api/recommendations/generate` — Invoke FastAPI GPT-4o generator.
* `PATCH /api/recommendations/:id/tasks` — Toggle task completed state.

### Community Hazard Reports APIs
* `GET /api/community/reports` — Fetch verified community hazards stream.
* `POST /api/community/reports` — Multipart form submit (image + GPS coordinates).

### AI Assistant (RAG) API
* `POST /api/assistant/chat` — Streaming response via Server-Sent Events (SSE).

---

## 4. Supabase RLS & Trigger Definitions
